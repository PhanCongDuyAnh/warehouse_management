...
function getFinanceMethods() {
    return {
        ...
        getFinancialMetrics() {
            ...
            const revenue = ((s.totalRevenue || 0) * 0.7) + realOrderRevenue;
            ...
            const scenario = s.scenario || 'normal';
            const heatFactor = scenario === 'heatwave' ? 1.8 : 1.0;
            const trafficFactor = scenario === 'peak_season' ? 1.5 : 1.0;

            const fuelCost = s.fuelCost || ((s.totalFuelConsumed || 0) * 25000 * trafficFactor); 
            const electricityCost = ((s.electricCost || 0) || 1250000) * heatFactor;
            ...
            if (typeof this.aiDiscountSuggestions === 'function') {
                opportunityLoss = this.aiDiscountSuggestions().reduce((sum, p) => {
                    const potentialRevenue = p.stock * p.baseSellPrice;
                    return sum + (potentialRevenue * (p.suggestedDiscount / 100));
                }, 0);
            }

            const lossCost = ((s.damagedItems || 0) * 2500000) + (inventoryLoss * 0.2); 
            ...
            const profitHeatmap = (this.shippingList || []).reduce((acc, ship) => {
                ...
                const orderCost = ((ship.fuelConsumed || 0) * 25000) + 150000; // Fuel + fixed labor per trip
            ...
            return {
                ...
                riskLevel: (s.activeStorm || s.activeTraffic) ? 'High' : (scenario !== 'normal' ? 'Medium' : 'Low')
            };
        },
        ...
        getAiInsights() {
            ...
            if (typeof this.aiDiscountSuggestions === 'function') {
                ...
                if (salvageable.length > 0) {
                    insights.push({
                        ...
                        desc: `Có ${salvageable.length} mặt hàng sắp hết hạn. Áp dụng giảm giá theo đề xuất AI có thể cứu vãn ${this.simFmt ? this.simFmt(finance.opportunityLoss) : finance.opportunityLoss} ₫.`,
                        impact: 'High',
                        color: 'orange'
                    });
                }
            ...
            const heatmapEntries = Object.entries(finance.profitHeatmap);
            if (heatmapEntries.length > 0) {
                const highCostRoute = heatmapEntries.find(([d, stats]) => stats.profit < 0);
                if (highCostRoute) {
                    insights.push({
                        type: 'logistics',
            ...
            if (s.scenario && s.scenario !== 'normal') {
                insights.push({
                    type: 'risk',
